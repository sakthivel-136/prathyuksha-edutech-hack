...
export default function FraudDetection() {
    ...
    const [stats, setStats] = useState({ total_downloads: 0, anomalies: 0, system_health: '99.9%' })
    ...
    const fetchData = async () => {
        try {
            ...
            const [anomRes, statRes] = await Promise.all([
                fetch(`${API_BASE}/api/fraud/anomalies`, { headers: getAuthHeaders() }),
                fetch(`${API_BASE}/api/fraud/stats`, { headers: getAuthHeaders() })
            ])
            if (anomRes.ok) setFlaggedTickets(await anomRes.json())
    ...
    const handleScan = async () => {
        ...
        try {
            const { API_BASE, getAuthHeaders } = await import('@/lib/api')
            await fetch(`${API_BASE}/api/fraud/scan`, { method: 'POST', headers: getAuthHeaders() })
            await fetchData()
        } catch (e) { console.error(e) }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                    <div className="vantage-card p-8 bg-[#001b5e] text-white border-none shadow-blue-900/20 relative overflow-hidden">
                        ...
                        <p className="text-slate-400 text-sm leading-relaxed mb-8">VANTAGE-EDU Guard AI uses Isolation Forest algorithms to detect out-of-distribution ticket behaviors across global endpoints.</p>
